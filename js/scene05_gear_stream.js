$(document).ready(function() {
    ...
    function animateFish($fish) {
        ...
        gsap.to($fish, {
            x: (Math.random() - 0.5) * ($fishArea.width() * 0.6), 
            y: (Math.random() - 0.5) * ($fishArea.height() * 0.6), 
            rotation: (Math.random() - 0.5) * 20, 
            duration: moveDuration,
            ease: "sine.inOut",
            onComplete: () => {
                if (fishingGameActive && $fish.parent().length) { 
                    animateFish($fish);
                }
            }
        });
    ...
    const sceneDialogues = [
        ...
        { 
            ...
            character: "Kitten",
            action: function(cb) {
                onFishingGameCompleteCallback = cb; 
                startFishingGame();
        ...
        { 
            ...
            action: function(cb) {
                ...
                gsap.timeline({ onComplete: cb })
                    ...
                    .to($otterSprite, { 
                        y: otterOriginalPos.y, 
                        x: otterOriginalPos.x - 30, 
                        autoAlpha: 1,
                        duration: 0.6,
                        delay: 0.5,
                        ease: "power2.out"
    ...
    const sceneData = {
        ...
        onSceneReady: function() {
            ...
            gsap.set($fishingGameContainer, {display: 'none'}); 
        }
    };
